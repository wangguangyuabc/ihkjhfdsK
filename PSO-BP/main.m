...
%%  划分训练集和测试集
temp = 1: 1: length(res);
nn = round(Rate*length(res));

P_train = res(temp(1: nn), 1: kim)';
T_train = res(temp(1: nn), kim+1)';
M = size(P_train, 2);

P_test = res(temp(nn+1: end), 1: kim)';
T_test = res(temp(nn+1: end), kim+1)';
N = size(P_test, 2);
